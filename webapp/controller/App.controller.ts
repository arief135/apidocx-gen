...
/**
 * @namespace oneflux.apiref.controller
 */
export default class App extends Controller {
	...
	private async _openEndpointDialog(key: string): Promise<void> {
		...
		if (!this._endpointDialog) {
			this._endpointDialog = (await this.loadFragment({ name: "oneflux.apiref.view.EditEndpoint" })) as Dialog;
	...
	public async onEditDocInfo(): Promise<void> {
		if (!this._docInfoDialog) {
			this._docInfoDialog = (await this.loadFragment({ name: "oneflux.apiref.view.EditDocInfo" })) as Dialog;
	...
	public onReset(): void {
		MessageBox.confirm("Reset to the original OneFlux reference? Your changes will be lost.", {
			onClose: (action: string) => {
				if (action === MessageBox.Action.OK) {
